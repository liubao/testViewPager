...
public class TabLayout extends LinearLayout {
    private String LOG_TAG = TabLayout.class.getSimpleName();
...
//        notifyDataSetChanged();
    ...
    public void notifyDataSetChanged() {
        removeAllViews();
        mChildCount = mPager.getAdapter().getCount();
        for (int i = 0; i < mChildCount; i++) {
            addTab(mPager.getAdapter().getPageTitle(i).toString());
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //测量每个子view，宽度
        for (int i = 0; i < mChildCount; i++) {
            getChildAt(i).measure(MeasureSpec.makeMeasureSpec(width / mChildCount, MeasureSpec.AT_MOST),
